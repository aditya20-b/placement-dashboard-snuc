...
export async function triggerWorkflow(flags: string[]): Promise<number> {
  ...
  const res = await fetch(
    `${BASE}/actions/workflows/${WORKFLOW_ID}/dispatches`,
    {
      ...
      body: JSON.stringify({
        ref: "main",
        inputs: { flags: flags.join(" "), spreadsheet_id: "" },
      }),
    }
  );
  ...
  // Poll up to 10 times (2s apart) for the new run ID
  for (let i = 0; i < 10; i++) {
